...
class ReidEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs, hazy=False):
        if hazy:
            self.hazy_pids.extend(inputs["targets"].numpy())
            self.hazy_camids.extend(inputs["camid"].numpy())
            self.hazy_features.append(outputs.cpu())
    ...
    def evaluate(self, hazy=False):
        if not hazy:
            ...
            self._results['R-5'] = cmc[4]
            self._results['R-10'] = cmc[9]
            self._results['R-20'] = cmc[19]
            ...
            features = torch.cat(self.hazy_features, dim=0)

            # query feature, person ids and camera ids
            query_features = features[:self._num_query]
            query_pids = np.asarray(self.hazy_pids[:self._num_query])
            query_camids = np.asarray(self.hazy_camids[:self._num_query])

            # gallery features, person ids and camera ids
            gallery_features = features[self._num_query:]
            gallery_pids = np.asarray(self.hazy_pids[self._num_query:])
            gallery_camids = np.asarray(self.hazy_camids[self._num_query:])
