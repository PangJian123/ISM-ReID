...
from fastreid.data import build_reid_test_loader, build_reid_train_loader, build_reid_paired_train_loader
from fastreid.evaluation import (DatasetEvaluator, ReidEvaluator,
                                 inference_on_dataset, print_csv_format)
from fastreid.modeling.meta_arch import build_model
from fastreid.solver import build_lr_scheduler, build_optimizer, build_dis_optimizer, build_dis_lr_scheduler
...
from . import hooks
from .train_loop import SimpleTrainer, HazySimpleTrainer, LoadSimpleTrainer

__all__ = ["default_argument_parser", "default_setup", "DefaultPredictor", "DefaultTrainer", "Hazytrainer", "LoadTrainer"]
...
def default_argument_parser():
    ...
    parser.add_argument("--config-file", default="/home/pj/fast-reid-master/configs/Hazy_Market1501/load_bagtricks_R50.yml", metavar="FILE", help="path to config file")
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    parser.add_argument(
        "--baseline",
        action="store_true",
        help="whether to train a baseline",
    )
    ...
    # parser.add_argument("--loss", default=["s_CrossEntropyLoss", ], nargs='+')
...
def default_setup(cfg, args):
    ...
    rank = comm.get_rank()
    setup_logger(output_dir, distributed_rank=rank, name="fvcore", args=args.info)
    logger = setup_logger(output_dir, distributed_rank=rank, args=args.info)
...
class DefaultTrainer(SimpleTrainer):
    ...
    def auto_scale_hyperparams(cfg, data_loader):
        ...
        # cfg.SOLVER.D_STEPS *= iters_per_epoch
...
class Hazytrainer(HazySimpleTrainer):
    """
    A trainer with default training logic. Compared to `SimpleTrainer`, it
    contains the following logic in addition:
    1. Create model, optimizer, scheduler, dataloader from the given config.
    ...
    def __init__(self, cfg):
        ...
        # Assume these objects must be constructed in this order.
        data_loader = self.build_paired_train_loader(cfg)
        t_data_loader = self.build_paired_train_loader(cfg, target=True)
        cfg = self.auto_scale_hyperparams(cfg, data_loader)
        model = self.build_model(cfg)
        # optimizer = self.build_optimizer(cfg, model)
        tea_optimizer = self.build_optimizer(cfg, model.teacher_net, model.teacher_heads)
        stu_optimizer = self.build_optimizer(cfg, model.student_net, model.student_heads)
        dis_optimizer = self.build_dis_optimizer(cfg, model.D_Net)
        ...
        super().__init__(model, data_loader, t_data_loader, tea_optimizer, stu_optimizer, dis_optimizer)

        self.tea_scheduler = self.build_lr_scheduler(cfg, tea_optimizer)
        self.stu_scheduler = self.build_lr_scheduler(cfg, stu_optimizer)
        self.dis_scheduler = self.build_dis_lr_scheduler(cfg, dis_optimizer)
        # Assume no other objects need to be checkpointed.
        # We can later make it checkpoint the stateful hooks
        self.checkpointer = Checkpointer(
            # Assume you want to save checkpoints together with logs/statistics
            ...
            cfg.OUTPUT_DIR,
            save_to_disk=comm.is_main_process(),
            tea_optimizer=tea_optimizer,
            stu_optimizer=stu_optimizer,
            tea_scheduler=self.tea_scheduler,
            stu_scheduler=self.stu_scheduler,
        )
        self.start_iter = 0
        if cfg.SOLVER.SWA.ENABLED:
            self.max_iter = cfg.SOLVER.MAX_ITER + cfg.SOLVER.SWA.ITER
    ...
    def build_hooks(self):
        ...
        ret = [
            hooks.IterationTimer(),
            hooks.DisLRScheduler(self.dis_optimizer, self.dis_scheduler),
            hooks.LRScheduler(self.tea_optimizer, self.tea_scheduler),
            hooks.LRScheduler(self.stu_optimizer, self.stu_scheduler),
        ]
    ...
    @classmethod
    def build_optimizer(cls, cfg, model, heads=None):
        """
        Returns:
            torch.optim.Optimizer:
        It now calls :func:`detectron2.solver.build_optimizer`.
        ...
        """
        if heads:
            return build_optimizer(cfg, model, heads=heads)
        else:
            return build_optimizer(cfg, model)


    def build_dis_optimizer(cls, cfg, model):
        """
        Returns:
            torch.optim.Optimizer:
        It now calls :func:`detectron2.solver.build_optimizer`.
        Overwrite it if you'd like a different optimizer.
        """
        return build_dis_optimizer(cfg, model)
    @classmethod
    def build_lr_scheduler(cls, cfg, optimizer):
        """
        It now calls :func:`detectron2.solver.build_lr_scheduler`.
    ...
    def build_dis_lr_scheduler(cls, cfg, optimizer):
        """
        It now calls :func:`detectron2.solver.build_lr_scheduler`.
        Overwrite it if you'd like a different scheduler.
        """
        return build_dis_lr_scheduler(cfg, optimizer)
    ...
    @classmethod
    def build_paired_train_loader(cls, cfg, target=False):
        """
        Returns:
            iterable
        It now calls :func:`fastreid.data.build_detection_train_loader`.
        ...
        logger = logging.getLogger(__name__)
        logger.info("Prepare training set")
        return build_reid_paired_train_loader(cfg, target=target)
    ...
    def auto_scale_hyperparams(cfg, data_loader):
        ...
        if cfg.MODEL.PARAM.BASEMODE:
            ...
            cfg.SOLVER.CHECKPOINT_PERIOD *= iters_per_epoch
            cfg.TEST.EVAL_PERIOD *= iters_per_epoch
            for i in range(len(cfg.SOLVER.D_STEPS)):
                cfg.SOLVER.D_STEPS[i] *= iters_per_epoch
        else:
            iters_per_epoch = len(data_loader.dataset) // (cfg.SOLVER.IMS_PER_BATCH * comm.get_world_size())
            cfg.MODEL.HEADS.NUM_CLASSES = data_loader.dataset.num_classes
            cfg.SOLVER.MAX_ITER = int(cfg.SOLVER.MAX_ITER * iters_per_epoch * 1.5)
            cfg.SOLVER.WARMUP_ITERS *= iters_per_epoch
            cfg.SOLVER.FREEZE_ITERS *= iters_per_epoch
            cfg.SOLVER.DELAY_ITERS *= iters_per_epoch
            for i in range(len(cfg.SOLVER.STEPS)):
            ...
            cfg.SOLVER.SWA.PERIOD *= iters_per_epoch
            cfg.SOLVER.CHECKPOINT_PERIOD *= iters_per_epoch
            cfg.TEST.EVAL_PERIOD *= 137
            for i in range(len(cfg.SOLVER.D_STEPS)):
                cfg.SOLVER.D_STEPS[i] *= iters_per_epoch
            cfg.MODEL.PARAM.Dis_iter = int(cfg.SOLVER.MAX_ITER * cfg.MODEL.PARAM.Dis_iter)
        ...
        logger.info(
            ...
            f"freeze_Iter={cfg.SOLVER.FREEZE_ITERS}, delay_Iter={cfg.SOLVER.DELAY_ITERS}, "
            f"step_Iter={cfg.SOLVER.STEPS}, ckpt_Iter={cfg.SOLVER.CHECKPOINT_PERIOD}, "
            f"eval_Iter={cfg.TEST.EVAL_PERIOD}, Dis_iter={cfg.MODEL.PARAM.Dis_iter}."
        )
...
class LoadTrainer(LoadSimpleTrainer):
    """
    A trainer with default training logic. Compared to `SimpleTrainer`, it
    contains the following logic in addition:
    1. Create model, optimizer, scheduler, dataloader from the given config.
    ...
    def __init__(self, cfg):
        ...
        data_loader = self.build_paired_train_loader(cfg)
        t_data_loader = self.build_paired_train_loader(cfg, target=True)
        cfg = self.load_auto_scale_hyperparams(cfg, data_loader)
        model = self.build_model(cfg)
        # optimizer = self.build_optimizer(cfg, model)
        stu_optimizer = self.build_optimizer(cfg, model.student_net, model.student_heads)
        dis_optimizer = self.build_dis_optimizer(cfg, model.D_net)
        ...
        super().__init__(model, data_loader, t_data_loader, dis_optimizer, stu_optimizer)
        ...
        self.checkpointer = Checkpointer(
            ...
            dis_scheduler=self.dis_scheduler,
            stu_optimizer=stu_optimizer,
            stu_scheduler=self.stu_scheduler,
        )
        self.start_iter = 0
    ...
    @staticmethod
    def load_auto_scale_hyperparams(cfg, data_loader):
        r"""
        This is used for auto-computation actual training iterations,
        because some hyper-param, such as MAX_ITER, means training epochs rather than iters,
        so we need to convert specific hyper-param to training iterations.
        ...
        iters_per_epoch = len(data_loader.dataset) // (cfg.SOLVER.IMS_PER_BATCH * comm.get_world_size())
        cfg.MODEL.HEADS.NUM_CLASSES = data_loader.dataset.num_classes
        cfg.SOLVER.MAX_ITER = cfg.SOLVER.MAX_ITER * iters_per_epoch + 20000
        cfg.SOLVER.WARMUP_ITERS *= iters_per_epoch
        cfg.SOLVER.FREEZE_ITERS *= iters_per_epoch
        cfg.SOLVER.DELAY_ITERS *= iters_per_epoch
        for i in range(len(cfg.SOLVER.STEPS)):
