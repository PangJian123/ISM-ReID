...
from .common import CommDataset, PairDataset
...
def build_reid_paired_train_loader(cfg, target=False):
    train_transforms = build_transforms(cfg, is_train=True)

    train_items = list()
    hazy_items = list()
    if target:
        for d in cfg.TDATASETS.NAMES:
            dataset = DATASET_REGISTRY.get(d)(root=_root, combineall=cfg.DATASETS.COMBINEALL)
            if comm.is_main_process():
                dataset.show_train()
            train_items.extend(dataset.train)
            hazy_items.extend(dataset.hazy_train)
    elif not target:
    ...
    train_set = PairDataset(train_items, hazy_items, train_transforms, relabel=True)
