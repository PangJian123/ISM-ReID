...
class PairDataset(object):
    ...
    def __init__(self, train, query, gallery, hazy_train, transform=None, mode='train',
                 combineall=False, verbose=True, **kwargs):
        self.train = train
        self.hazy_train = hazy_train
        self.query = query
        # self.hazy_query = hazy_query
        self.gallery = gallery
        # self.hazy_gallery = hazy_gallery
        self.transform = transform
        self.mode = mode
        self.combineall = combineall
        self.verbose = verbose
        ...
        self.num_train_pids = self.get_num_pids(self.train)
        self.num_hazy_train_pids = self.get_num_pids(self.hazy_train)
        self.num_train_cams = self.get_num_cams(self.train)
        self.num_hazy_train_cams = self.get_num_cams(self.hazy_train)
        ...
        if self.mode == 'train':
            self.data = self.train
            self.hazy_data = self.hazy_train
        elif self.mode == 'query':
            self.data = self.query
        elif self.mode == 'gallery':
            self.data = self.gallery
...
class ImagePairDataset(PairDataset):
    """A base class representing ImageDataset.
    All other image datasets should subclass it.
    ``__getitem__`` returns an image given index.
    It will return ``img``, ``pid``, ``camid`` and ``img_path``
    ...
    def __init__(self, train, query, gallery, hazy_train, **kwargs):
        super(ImagePairDataset, self).__init__(train, query, gallery, hazy_train, **kwargs)

    def show_train(self):
        logger = logging.getLogger(__name__)
        num_train_pids, num_train_cams = self.parse_data(self.train)
        num_hazy_train_pids, num_hazy_train_cams = self.parse_data(self.hazy_train)
        logger.info('=> Loaded {}'.format(self.__class__.__name__))
        logger.info('  ----------------------------------------')
        logger.info('  subset   | # ids | # images | # cameras')
        logger.info('  ----------------------------------------')
        logger.info('  train    | {:5d} | {:8d} | {:9d}'.format(num_train_pids, len(self.train), num_train_cams))
        logger.info('  ----------------------------------------')
        logger.info('  hazy_train    | {:5d} | {:8d} | {:9d}'.format(num_hazy_train_pids, len(self.hazy_train), num_hazy_train_cams))
        logger.info('  ----------------------------------------')
